...
const statusColors = {
  ...
  "In Bearbeitung": "bg-cyan-50 text-cyan-500 border-blue-500/30",
  ...
  return (
    <div className="space-y-8 animate-fade-in" data-testid="instandhaltung-page">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 font-manrope">Instandhaltung</h1>
          <p className="text-slate-400 mt-1">{tickets.length} Wartungsaufgaben</p>
        </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 mt-4">
              <div>
                ...
                <Select value={formData.property_id} onValueChange={(v) => setFormData({ ...formData, property_id: v })}>
                  <SelectTrigger className="mt-1 bg-white border-slate-200 text-slate-900" data-testid="ticket-property-select">
                  ...
                  <Select value={formData.priority} onValueChange={(v) => setFormData({ ...formData, priority: v })}>
                    ...
                    <SelectContent className="bg-white border-slate-200">
                      {priorityOptions.map((p) => (
                        <SelectItem key={p} value={p} className="text-slate-900 hover:bg-slate-50">{p}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-slate-500">Handwerker zuweisen</Label>
                  <Select value={formData.assigned_to_id} onValueChange={(v) => setFormData({ ...formData, assigned_to_id: v })}>
                    <SelectTrigger className="mt-1 bg-white border-slate-200 text-slate-900">
                      <SelectValue placeholder="Optional" />
                    </SelectTrigger>
                    <SelectContent className="bg-white border-slate-200">
                      <SelectItem value="none" className="text-slate-900 hover:bg-slate-50">Nicht zugewiesen</SelectItem>
                      {handwerker.map((h) => (
                        <SelectItem key={h.id} value={h.id} className="text-slate-900 hover:bg-slate-50">
                ...
                <div>
                  <Label className="text-slate-500">Geplant für</Label>
                  <Input
                    type="date"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-slate-500">Kosten (€)</Label>
                  <Input
                    type="number"
                    value={formData.cost}
                ...
                <div className="flex items-end gap-3">
                  ...
                  <Label className="text-slate-500 pb-2">Wiederkehrend</Label>
                </div>
              </div>
              {formData.is_recurring && (
                <div>
                  <Label className="text-slate-500">Intervall (Tage)</Label>
                  <Input
                    type="number"
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4" data-testid="maintenance-filters">
        <div className="flex flex-col md:flex-row gap-4">
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-[180px] bg-white border-slate-200 text-slate-900" data-testid="filter-status">
              <Filter className="w-4 h-4 mr-2 text-slate-400" />
              <SelectValue placeholder="Alle Status" />
            </SelectTrigger>
            <SelectContent className="bg-white border-slate-200">
              <SelectItem value="all" className="text-slate-900 hover:bg-slate-50">Alle Status</SelectItem>
              {statusOptions.map((s) => (
                <SelectItem key={s} value={s} className="text-slate-900 hover:bg-slate-50">{s}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <Select value={priorityFilter} onValueChange={setPriorityFilter}>
            <SelectTrigger className="w-[180px] bg-white border-slate-200 text-slate-900" data-testid="filter-priority">
              <AlertTriangle className="w-4 h-4 mr-2 text-slate-400" />
              <SelectValue placeholder="Alle Prioritäten" />
            </SelectTrigger>
            <SelectContent className="bg-white border-slate-200">
              <SelectItem value="all" className="text-slate-900 hover:bg-slate-50">Alle Prioritäten</SelectItem>
              {priorityOptions.map((p) => (
                <SelectItem key={p} value={p} className="text-slate-900 hover:bg-slate-50">{p}</SelectItem>
              ))}
            </SelectContent>
          ...
          {(statusFilter || priorityFilter) && (
            <Button variant="ghost" onClick={() => { setStatusFilter(""); setPriorityFilter(""); }} className="text-slate-500 hover:text-slate-900">
      ...
      {tickets.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm p-12 text-center" data-testid="no-tickets">
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-2">Keine Wartungsaufgaben</h3>
          <p className="text-slate-400 mb-4">Erstellen Sie Ihr erstes Ticket</p>
            ...
            >
              <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <h3 className="text-lg font-semibold text-slate-900 font-manrope">{ticket.title}</h3>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => handleEdit(ticket)} className="text-slate-500 hover:text-slate-900 hover:bg-slate-50">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent className="bg-white border-slate-200">
                      <AlertDialogHeader>
                        <AlertDialogTitle className="text-slate-900">Ticket löschen?</AlertDialogTitle>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction onClick={() => handleDelete(ticket.id)} className="bg-red-500 hover:bg-red-600 text-slate-900">Löschen</AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                </div>
