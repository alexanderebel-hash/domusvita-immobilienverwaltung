...
export default function Immobilien() {
  ...
  const getStatusClass = (status) => {
    switch (status) {
      ...
      default:
        return "bg-slate-50 text-gray-700";
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in" data-testid="immobilien-page">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 font-manrope">Immobilien</h1>
          <p className="text-slate-400 mt-1">{properties.length} Objekte in Ihrem Portfolio</p>
        </div>
          ...
          <DialogContent className="bg-white border-slate-200 text-slate-900 max-w-lg">
            ...
            <form onSubmit={handleCreateProperty} className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label className="text-slate-500">Stadt *</Label>
                  <Input
                ...
                <div>
                  <Label className="text-slate-500">PLZ</Label>
                  <Input
                ...
                <div>
                  ...
                  >
                    <SelectTrigger className="mt-1 bg-white border-slate-200 text-slate-900" data-testid="property-type-select">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  >
                    <SelectTrigger className="mt-1 bg-white border-slate-200 text-slate-900" data-testid="property-status-select">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="bg-white border-slate-200">
                      {statusOptions.map((status) => (
                        <SelectItem key={status} value={status} className="text-slate-900 hover:bg-slate-50">
                          {status}
                        </SelectItem>
                      ))}
                    </SelectContent>
                ...
                <div>
                  <Label className="text-slate-500">Einheiten</Label>
                  <Input
                    type="number"
                    min="1"
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex flex-wrap gap-3">
            <Select value={filters.type} onValueChange={(value) => setFilters({ ...filters, type: value })}>
              <SelectTrigger className="w-[160px] bg-white border-slate-200 text-slate-900" data-testid="filter-type">
                <Filter className="w-4 h-4 mr-2 text-slate-400" />
                <SelectValue placeholder="Typ" />
              </SelectTrigger>
              <SelectContent className="bg-white border-slate-200">
                <SelectItem value="all" className="text-slate-900 hover:bg-slate-50">Alle Typen</SelectItem>
                {propertyTypes.map((type) => (
                  <SelectItem key={type} value={type} className="text-slate-900 hover:bg-slate-50">
                    {type}
                  </SelectItem>
            ...
            <Select value={filters.city} onValueChange={(value) => setFilters({ ...filters, city: value })}>
              <SelectTrigger className="w-[160px] bg-white border-slate-200 text-slate-900" data-testid="filter-city">
                <MapPin className="w-4 h-4 mr-2 text-slate-400" />
                <SelectValue placeholder="Stadt" />
              </SelectTrigger>
              <SelectContent className="bg-white border-slate-200">
                <SelectItem value="all" className="text-slate-900 hover:bg-slate-50">Alle Städte</SelectItem>
                {cities.map((city) => (
                  <SelectItem key={city} value={city} className="text-slate-900 hover:bg-slate-50">
                    {city}
                  </SelectItem>
                ))}
              </SelectContent>
            ...
            <Select value={filters.status} onValueChange={(value) => setFilters({ ...filters, status: value })}>
              <SelectTrigger className="w-[160px] bg-white border-slate-200 text-slate-900" data-testid="filter-status">
          ...
          <div className="flex gap-1 bg-white rounded-lg p-1">
            <button
              ...
              className={`p-2 rounded-md transition-all ${viewMode === "grid" ? "bg-slate-50 text-slate-900" : "text-slate-400 hover:text-slate-900"}`}
            ...
            <button
              ...
              className={`p-2 rounded-md transition-all ${viewMode === "list" ? "bg-slate-50 text-slate-900" : "text-slate-400 hover:text-slate-900"}`}
      ...
      {filteredProperties.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm p-12 text-center" data-testid="no-properties">
          ...
          <h3 className="text-lg font-medium text-slate-900 mb-2">Keine Immobilien gefunden</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" data-testid="properties-grid">
          {filteredProperties.map((property, index) => (
            ...
            >
              ...
              {/* Content */}
              <div className="p-5">
                <h3 className="text-lg font-semibold text-slate-900 font-manrope mb-2 line-clamp-1">
                  {property.name}
                </h3>
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                  ...
                  <Button variant="ghost" size="sm" className="text-cyan-500 hover:text-cyan-300 hover:bg-cyan-500/10 p-0">
                    Details →
                  </Button>
                </div>
              </div>
        ...
        <div className="space-y-3" data-testid="properties-list">
          {filteredProperties.map((property, index) => (
            <div
              key={property.id}
              onClick={() => navigate(`/immobilien/${property.id}`)}
              className="bg-white rounded-2xl shadow-sm p-4 flex items-center gap-4 cursor-pointer hover:bg-slate-50 transition-all opacity-0 animate-fade-in"
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-slate-900 font-medium font-manrope truncate">{property.name}</h3>
