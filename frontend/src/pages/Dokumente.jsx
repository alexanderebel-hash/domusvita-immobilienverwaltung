...
const categoryColors = {
  Vertrag: "bg-cyan-50 text-cyan-500 border-blue-500/30",
  ...
  Sonstiges: "bg-slate-500/20 text-slate-400 border-gray-500/30",
};
  ...
  return (
    <div className="space-y-8 animate-fade-in" data-testid="dokumente-page">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 font-manrope">Dokumente</h1>
          <p className="text-slate-400 mt-1">{documents.length} Dokumente verwalten</p>
        </div>
            ...
            <form onSubmit={handleUpload} className="space-y-4 mt-4">
              <div>
                ...
                <Select value={uploadData.property_id} onValueChange={(v) => setUploadData({ ...uploadData, property_id: v })}>
                  <SelectTrigger className="mt-1 bg-white border-slate-200 text-slate-900" data-testid="upload-property-select">
              ...
              <div>
                ...
                <Select value={uploadData.category} onValueChange={(v) => setUploadData({ ...uploadData, category: v })}>
                  <SelectTrigger className="mt-1 bg-white border-slate-200 text-slate-900" data-testid="upload-category-select">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-white border-slate-200">
                    {categoryOptions.map((c) => (
                      <SelectItem key={c} value={c} className="text-slate-900 hover:bg-slate-50">{c}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div>
                <Label className="text-slate-500">Datei *</Label>
                <div 
                  className="mt-1 border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:border-white/40 transition-colors cursor-pointer"
                ...
                >
                  ...
                  {uploadData.file ? (
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className="text-slate-900">{uploadData.file.name}</span>
                      <Button
                        type="button"
                        size="sm"
                        variant="ghost"
                    ...
                    <>
                      ...
                      <p className="text-slate-500">Klicken oder Datei hierher ziehen</p>
                      <p className="text-slate-400 text-sm mt-1">PDF, DOC, XLS, JPG, PNG</p>
                    </>
                  )}
                </div>
              </div>
              <div className="flex justify-end gap-3 pt-4">
                <Button type="button" variant="outline" onClick={() => setIsDialogOpen(false)} className="bg-white border-slate-200 text-slate-900 hover:bg-slate-50">
                  Abbrechen
                </Button>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4" data-testid="dokumente-filters">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <Select value={propertyFilter} onValueChange={setPropertyFilter}>
            <SelectTrigger className="w-[200px] bg-white border-slate-200 text-slate-900" data-testid="filter-property">
              <Building2 className="w-4 h-4 mr-2 text-slate-400" />
              <SelectValue placeholder="Alle Immobilien" />
            </SelectTrigger>
            <SelectContent className="bg-white border-slate-200">
              <SelectItem value="all" className="text-slate-900 hover:bg-slate-50">Alle Immobilien</SelectItem>
              {properties.map((p) => (
                <SelectItem key={p.id} value={p.id} className="text-slate-900 hover:bg-slate-50">{p.name}</SelectItem>
              ))}
            </SelectContent>
          ...
          <Select value={categoryFilter} onValueChange={setCategoryFilter}>
            <SelectTrigger className="w-[180px] bg-white border-slate-200 text-slate-900" data-testid="filter-category">
              <Filter className="w-4 h-4 mr-2 text-slate-400" />
              <SelectValue placeholder="Alle Kategorien" />
            </SelectTrigger>
            <SelectContent className="bg-white border-slate-200">
              <SelectItem value="all" className="text-slate-900 hover:bg-slate-50">Alle Kategorien</SelectItem>
              {categoryOptions.map((c) => (
                <SelectItem key={c} value={c} className="text-slate-900 hover:bg-slate-50">{c}</SelectItem>
              ))}
            </SelectContent>
          ...
          {(propertyFilter || categoryFilter || searchTerm) && (
            <Button variant="ghost" onClick={() => { setPropertyFilter(""); setCategoryFilter(""); setSearchTerm(""); }} className="text-slate-500 hover:text-slate-900">
      ...
      {filteredDocuments.length === 0 ? (
        <div className="bg-white rounded-2xl shadow-sm p-12 text-center" data-testid="no-documents">
          ...
          <p className="text-slate-400 mb-4">Laden Sie Ihr erstes Dokument hoch</p>
            ...
            return (
              <div key={property.id} className="space-y-4" data-testid={`documents-group-${property.id}`}>
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-lg font-semibold text-slate-900 font-manrope">{property.name}</h2>
                  <span className="text-sm text-slate-400">({propDocs.length} Dokumente)</span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {propDocs.map((doc, idx) => {
                    ...
                    return (
                      <div
                        key={doc.id}
                        className="bg-white rounded-2xl shadow-sm p-4 hover:bg-slate-50 transition-all opacity-0 animate-fade-in"
                      ...
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="text-slate-900 font-medium truncate">{doc.name}</h3>
                        ...
                        <div className="flex gap-2 mt-4 pt-3 border-t border-slate-200">
                          <Button
                            ...
                            className="flex-1 text-slate-500 hover:text-slate-900 hover:bg-slate-50"
                          >
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent className="bg-white border-slate-200">
                              <AlertDialogHeader>
                                <AlertDialogTitle className="text-slate-900">Dokument löschen?</AlertDialogTitle>
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => handleDelete(doc.id)} className="bg-red-500 hover:bg-red-600 text-slate-900">Löschen</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </div>
